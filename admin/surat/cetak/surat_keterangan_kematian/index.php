<?php 
	include ('../../permintaan_surat/konfirmasi/part/akses.php');
  	...
  	$id = $_GET['id'];
  	$qCek = mysqli_query($connect,"SELECT penduduk.*, surat_keterangan_kematian.* FROM penduduk LEFT JOIN surat_keterangan_kematian ON surat_keterangan_kematian.nik = penduduk.nik WHERE surat_keterangan_kematian.id_sk='$id'");
  	while($row = mysqli_fetch_array($qCek)){
        ...
        foreach($qTampilDesa as $rows){
			...
			$id_pejabat_desa = $row['id_pejabat_desa'];
		  	$qCekPejabatDesa = mysqli_query($connect,"SELECT pejabat_desa.jabatan, pejabat_desa.nama_pejabat_desa FROM pejabat_desa LEFT JOIN surat_keterangan_kematian ON surat_keterangan_kematian.id_pejabat_desa = pejabat_desa.id_pejabat_desa WHERE surat_keterangan_kematian.id_pejabat_desa = '$id_pejabat_desa' AND surat_keterangan_kematian.id_sk='$id'");
...
<div>
	...
	<div id="isi3">
		...
		<table width="100%">
			<tr>
				<td class="indentasi">Berdasarkan Surat Pengantar RT. <?php echo $row['rt']; ?>/<?php echo $row['rw']; ?> Nomor <?php echo $row['keperluan']; ?><?php echo $row['desa']; ?> <?php echo $row['kecamatan']; ?> <?php echo $row['kota']; ?> tanggal...... Bahwa nama tersebut diatas 
				berdomisili di <?php echo $row['domisil']; ?> menerangkan bahwa nama tersebut diatas telah meninggal dunia Pada Hari <?php echo $row['date_death']; ?> di <?php echo $row['location_death']; ?> karena <?php echo $row['karena']; ?>.</td>
			</tr>
		</table><br>
		<table width="100%">
			<tr>
