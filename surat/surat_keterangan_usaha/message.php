...
$sql = "SELECT no_hp, id_sku FROM surat_keterangan_usaha WHERE id_sku = '$id'";
$result = mysqli_query($connect, $sql);

if ($result && mysqli_num_rows($result) > 0) {
    $data = mysqli_fetch_assoc($result);
    $no_hp = '6281289818521';
    $no_surat = $data['id_sku'];
} else {
...
<div class="container">
    ...
    if (!file_exists($flagFile)) {
        ...
        if (isset($response_data['status']) && $response_data['status'] == 'success') {
            ...
            $SQL = "UPDATE surat_keterangan_usaha SET FLAG_SMS = 'N' WHERE id_sku = '$id'";
