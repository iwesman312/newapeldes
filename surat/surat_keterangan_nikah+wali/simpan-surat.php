<?php
    ...
    if (isset($_POST['submit'])){
        $jenis_surat = "Surat Keterangan Nikah + Surat Wali";
        $nik = $_POST['fnik'];
        ...
        $wali = addslashes($_POST['fwali']);
        $status_surat = "PENDING";
        $id_profil_desa = "1";
        $FLAG_SMS = "N";

        $qTambahSurat = "INSERT INTO surat_keterangan_nikah_wali (jenis_surat, nik, nik_ayah, nik_ibu, status_surat, status_nik, nama_terdahulu, 
        ...
        alamat_mempelai, kewarganegaraan_mempelai, FLAG_SMS, ortu_mempelai,wali) VALUES('$jenis_surat', '$nik', '$nikayah', '$nikibu', '$status_surat', '$status','$namadulu',
        '$namamempelai','$id_profil_desa','$tanggal_nikah','$tempat_nikah','$NO_HP','$jkmempelai',
        '$ttlmempelai','$agamamempelai','$nikmempelai','$pekerjaanmempelai','$alamatmempelai','$kwnmempelai','$FLAG_SMS','$ortu','$wali')";
    ...
    if ($TambahSurat) {
        ...
        $url = "http://localhost/desa/surat/surat_keterangan_nikah+wali/message.php?id=$id_sk"; // Sesuaikan dengan URL message.php di server Anda
        ...
        header("location:../surat_keterangan_nikah+wali/message.php?id=$id_sk");
        exit();
    } else {
        echo "<script LANGUAGE='JavaScript'>window.alert('Gagal menyimpan data!');</script>";
    }
}
